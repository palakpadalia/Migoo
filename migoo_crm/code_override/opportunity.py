...
class Opportunity(TransactionBase, CRMNote):
    ...
    def after_insert(self):
        if self.opportunity_from == "Lead":
            ...
            if frappe.db.get_single_value("CRM Settings", "carry_forward_communication_and_comments"):
                ...
                link_communications(self.opportunity_from,
                                    self.party_name, self)
            ...
            if not self.get(field) and frappe.db.field_exists(self.opportunity_from, field):
                try:
                    value = frappe.db.get_value(
                        self.opportunity_from, self.party_name, field)
    ...
    def set_exchange_rate(self):
        ...
        if not self.conversion_rate or self.conversion_rate == 1.0:
            self.conversion_rate = get_exchange_rate(
                self.currency, company_currency, self.transaction_date)
    ...
    def update_prospect(self):
        ...
        if self.opportunity_from == "Prospect" and self.party_name:
            ...
            prospect_name = frappe.db.get_value(
                "Prospect Lead", {"lead": self.party_name}, "parent")
    ...
    def disable_lead(self):
        if self.opportunity_from == "Lead":
            frappe.db.set_value("Lead", self.party_name, {
                                "disabled": 1, "docstatus": 1})
    ...
    def make_new_lead_if_required(self):
        ...
        if (not self.get("party_name")) and self.contact_email:
            ...
            lead_name = frappe.db.get_value(
                "Lead", {"email_id": self.contact_email})
            if not lead_name:
                ...
                lead = frappe.get_doc(
                    {"doctype": "Lead", "email_id": self.contact_email,
    ...
    def has_active_quotation(self):
        if not self.get("items", []):
            return frappe.get_all(
                "Quotation",
                {"opportunity": self.name, "status": (
                    "not in", ["Lost", "Closed"]), "docstatus": 1},
                "name",
            )
        else:
        ...
        if not self.get("items", []):
            return frappe.get_all(
                "Quotation", {"opportunity": self.name,
                              "status": "Ordered", "docstatus": 1}, "name"
            )
        else:
            return frappe.db.sql(
                """
    ...
    def validate_item_details(self):
        ...
        for d in self.items:
            ...
            item = frappe.db.get_value(
                "Item", d.item_code, item_fields, as_dict=True)
...
def make_customer(source_name, target_doc=None):
    ...
    doclist = get_mapped_doc(
        ...
        source_name,
        {
            "Opportunity": {
                "doctype": "Customer",
                "field_map": {"currency": "default_currency",
...
def auto_close_opportunity():
    ...
    auto_close_after_days = (
        frappe.db.get_single_value(
            "CRM Settings", "close_opportunity_after_days") or 15
    )
    ...
    opportunities = (
        ...
        .select(table.name)
        .where(
            (table.modified < (Now() - Interval(days=auto_close_after_days))
             ) & (table.status == "Replied")
        )
    ).run(pluck=True)
...
def make_opportunity_from_communication(communication, company, ignore_communication_links=False):
    ...
    if not lead:
        lead = make_lead_from_communication(
            communication, ignore_communication_links=True)
    ...
    link_communication_to_document(
        doc, "Opportunity", opportunity.name, ignore_communication_links)
