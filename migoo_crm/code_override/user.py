...
class User(Document):
    ...
    def onload(self):
        ...
        self.set_onload("all_modules", [m.get("module_name")
    ...
    def validate(self):
        ...
        if (self.name not in ["Administrator", "Guest"]) and (
            ...
            self.set_social_login_userid(
                "frappe", frappe.generate_hash(length=39))

    def validate_roles(self):
        if self.role_profile_name:
            role_profile = frappe.get_doc(
                "Role Profile", self.role_profile_name)
            self.set("roles", [])
    ...
    def validate_allowed_modules(self):
        if self.module_profile:
            module_profile = frappe.get_doc(
                "Module Profile", self.module_profile)
    ...
    def on_update(self):
        ...
        if self.name not in ("Administrator", "Guest") and not self.user_image:
            frappe.enqueue(
                "frappe.core.doctype.user.user.update_gravatar", name=self.name, now=now)
    ...
    def add_system_manager_role(self):
        ...
        if (
            ...
            self.append("roles", {"doctype": "Has Role",
                        "role": "System Manager"})
    ...
    def set_system_user(self):
        ...
        user_type_mapper = {
            "Administrator": "System User", "Guest": "Website User"}
    ...
    def password_reset_mail(self, link):
        self.send_login_mail(_("Password Reset"), "password_reset", {
                             "link": link}, now=True)
    ...
    def send_welcome_mail_to_user(self):
        ...
        if method:
            subject = _("Ready to scale your business to new heights?")
        # if not subject:
    ...
    def send_login_mail(self, subject, template, add_args, now=None):
        ...
        sender = (
            frappe.session.user not in STANDARD_USERS and get_formatted_email(
    ...
    def on_trash(self):
        ...
        # delete events
        frappe.db.delete(
            "Event", {"owner": self.name, "event_type": "Private"})
        ...
        frappe.qb.update(table).where(
            table.user == self.name).set(table.user, None).run()

        # delete notification settings
        frappe.delete_doc("Notification Settings",
    ...
    def after_rename(self, old_name, new_name, merge=False):
        ...
        if frappe.db.exists("Notification Settings", old_name):
            frappe.rename_doc("Notification Settings", old_name,
                              new_name, force=True, show_alert=False)
    ...
    def remove_all_roles_for_guest(self):
        if self.name == "Guest":
            self.set("roles", list(
                {d for d in self.get("roles") if d.role == "Guest"}))

    def remove_disabled_roles(self):
        disabled_roles = [d.name for d in frappe.get_all(
            "Role", filters={"disabled": 1})]
    ...
    def validate_username(self):
        ...
        if self.username_exists():
            if self.user_type == "System User":
                frappe.msgprint(
                    _("Username {0} already exists").format(self.username))
    ...
    def password_strength_test(self):
        ...
        if self.__new_password:
            user_data = (self.first_name, self.middle_name,
                         self.last_name, self.email, self.birth_date)
            result = test_password_strength(
                self.__new_password, "", None, user_data)
    ...
    def username_exists(self, username=None):
        return frappe.db.get_value(
            "User", {"username": username or self.username,
                     "name": ("!=", self.name)}
        )
    ...
    def validate_user_email_inbox(self):
        ...
        email_accounts = [
            user_email.email_account for user_email in self.user_emails]
    ...
    def find_by_credentials(cls, user_name: str, password: str, validate_password: bool = True):
        ...
        login_with_username = cint(
            frappe.db.get_single_value(
                "System Settings", "allow_login_using_user_name")
        )
        ...
        users = frappe.get_all(
            "User", fields=["name", "enabled"], or_filters=or_filters, limit=1)
        if not users:
            return
...
def _get_user_for_update_password(key, old_password):
    ...
    if key:
        ...
        result.user, last_reset_password_key_generated_on = user or (
            None, None)
        if result.user:
...
def user_query(doctype, txt, searchfield, start, page_len, filters):
	...
	""".format(
            ...
            standard_users=", ".join(frappe.db.escape(u)
...
def get_permission_query_conditions(user):
    if user == "Administrator":
        ...
        return """(`tabUser`.name not in ({standard_users}))""".format(
            standard_users=", ".join(frappe.db.escape(user)
...
def handle_password_test_fail(feedback: dict):
    ...
    frappe.throw(msg=" ".join([warning] + suggestions),
